...
We can see how RNNs struggle to classify sentences in the training dataset, thus their accuracy drops. Linear layers don't maintain word context and their addition reduces the accuracy even further. This is expected considering that RNNs struggle wth word context tracking, especially as sentences grow past 20 words.

On the other hand, LSTM networks are able to maintain high accuracies on these larger sentences attributed to their memory mechanism which allows them to maintain word context.

Furthermore, we can see a slight increase in training times due to the doubling of the sentence size - more processing is required.
...
I used the same results extraction function, defined above. Details of the implementations of the rest of the neural networks are provided in the notebook (\lstinline{part-C.ipynb}).
