...
\HRule \\[0.4cm]
{\huge 1η Σειρά Ασκήσεων 2025-2026\\}
\HRule \\[0.4cm]
\end{center}
...
$5615/11017=50\%$ and $5072/10445=48\%$ so half of the vocabulary appears only once in the tokens. However, BERT does not follow this law $2915/8544=34\%$.
...
In each row, $k=0.01$ results in a significantly better (lower) perplexity. $k=1$ proves to be a much more aggressive smoothing method which overshadows the rest of the probabilities.

The lowercase column appears to perform worse than the original one. I imagine this is happening because ``<UNK`` gets a higher frequency in the later making it much easier to appear in the vocabulary than in the former. This is happening because more words cross the cutover threshold (3) because they're lowercased. In this case, the vocabulary is larger and also more complex which forces the model to have to guess the next word instead of assigning ``<UNK>``, thus increasing perplexity.

We can see that bigrams perform better than trigrams in every case. This is expected because our dataset is too small to create a large enough vocabulary of every possible trigram combination that we encounter in the test dataset.

The Abstract Digits column performs better (lower perplexity) than the Lowercase column due to digit abstraction and vocabulary shrinkage. All digits collapse into combination of ``\#`` which the model does not have to learn. This makes it easier for the model to predict digits in a sentence.
